...
// FIX: Frames will complete at 90% of the scroll, giving a buffer for the final text.
const FRAME_ANIMATION_END_PROGRESS = 0.9; 
...
ScrollTrigger.create({
    ...
    onUpdate: (self) => {
        ...
        // Calculate the frame progress, capping the input at FRAME_ANIMATION_END_PROGRESS
        const frameAnimationProgress = Math.min(self.progress / FRAME_ANIMATION_END_PROGRESS, 1);

        // Update frame using the capped progress
        const frameIndex = Math.min(
            Math.floor(frameAnimationProgress * FRAME_COUNT),
            FRAME_COUNT - 1
        );
        ...
        // Update text animation using the full progress
